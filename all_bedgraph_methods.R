PCHiC_bedgraph_calc <- function(scores,
                                ...
                                output_bed = "temp.bed"){
  library(data.table)
  pchic_data = data.frame(fread("../processed_data/processed_data/PCHiC_peak_matrix_cutoff5.tsv", header=T))
...
ABC_intergenic_bedgraph_calc <- function(scores,
  ...
  df_pre = data.frame(fread(paste0("../processed_data/processed_data","/",
...
ABC_Road_bedgraph_calc <- function(scores,
  ...
  if(tissuename != "ALL"){
    ...
    if(tissuename == "BLD"){
      tissuenames2 = as.character(read.table("../processed_data/processed_data/ABC.listbloodQC.txt", header=F)[,1])
    }
    if(tissuename == "LNG"){
      tissuenames2 = c("lung")
    }
  ...
  roadmap_meta = read.delim("../processed_data/processed_data/Roadmap_map_EID_names.txt",
                            header=F)
  ...
  Enhancer = c()
  for(ee in Road_ids){
    temp = read.table(paste0("../processed_data/extras/RoadmapLinks/",
                             "links_", ee, "_7_2.5.txt"))
    temp2 = read.table(paste0("../processed_data/extras/RoadmapLinks/",
                              "links_", ee, "_6_2.5.txt"))
    Enhancer = rbind(Enhancer, temp[,1:4], temp2[,1:4])
    cat("We processed file:", ee, "\n")
  }

  geneanno = read.csv("../processed_data/processed_data/gene_anno_unique_datefix.txt", sep = "\t")
...
Yoshida_bedgraph_calc <- function(scores,
  ...
  df = data.frame(fread(paste0("../processed_data/processed_data",
                               "/", "Yoshida_correlated_summits_per_gene_inside100KB_sort.bed")))
  unique_genes = as.character(unique(df[,4]))
  gene_orthologs = read.table("../processed_data/processed_data/Orthologs_Yoshida_Mouse_Human.txt", header=T)
...
eQTLCPP_bedgraph_calc = function(scores,
  ...
  Enhancer = read.table("../processed_data/processed_data/eQTL_Blood_CPP.txt",
                        header=F)
...
Roadmap_Enhancer_bedgraph_calc = function(scores,
  ...
  Enhancer = read.table("../processed_data/processed_data/Roadmap_Enhancers_Blood.txt",
                        header=F)
  matched_ids = match(Enhancer[,4], names(scores))
  temp = as.numeric(scores)[matched_ids]
  temp[is.na(temp)] = 0
